...
- **The Devin CLI bridge was unusable from inside Devin Desktop.** Two independent causes, both silent. First, `devin auth status` reports "Not logged in" when Devin Desktop authenticates the CLI over ACP (`method_id=windsurf-api-key`) rather than through the CLI's own credential store; detection now also honours `WINDSURF_API_KEY`, which the CLI accepts as the same credential. Second — and this masked a *genuine* `devin auth login` — the dashboard is normally launched from a terminal inside Devin Desktop, which exports `ACP_BACKEND`. With it set the CLI switches to ACP mode, where "ACP host is the sole source of credentials. Local CLI credentials (env vars, on-disk REPL store) will NOT be used", so a correctly logged-in CLI still reported "Not logged in" and was skipped. The bridge now strips `ACP_BACKEND` from the environment it spawns.
- **MCP calls from the spawned CLI were rejected.** `--permission-mode auto` approves read-only tools but not MCP calls, so the agent answered "rejected a tool call that requires confirmation" and gave up; the CLI's own suggestion is `--permission-mode dangerous`, which auto-approves *everything* — far too broad for answering a question about a codebase. The bridge now generates a config granting exactly `mcp__sprang__*` and passes it with `--config`.
- **A failed session resume surfaced as a failed question.** Resuming a recorded session can fail for reasons unrelated to the question (`failed to start ACP agent session`), which degraded the answer to the manual relay. A failed resume now clears the session and retries once as a fresh one, restarting the turn counter rather than carrying the abandoned thread forward.
- **The Ask Agent panel could hang forever on an installed-but-broken CLI.** Bridge detection can only see whether a binary answers `--version`; it cannot see a revoked OAuth token, an unsupported model, or a rate limit (Claude's own `claude auth status` reports `loggedIn: true` for a token the API has revoked). The background ask functions failed silently, so a selected-but-failing CLI left the dashboard polling indefinitely. Any CLI failure now degrades to the `relay` bridge, surfacing the underlying error together with a paste-able prompt so the question can still be answered.
- **The CLI bridges inherited stdin from the server and hung.** `claude`, `copilot` and `devin` all wait for piped input when stdin is inherited from a long-lived process, then exit non-zero (`no stdin data received in 3s`) — so the dashboard bridge failed even with valid credentials. All six spawn sites now pass `stdio: ['ignore', 'pipe', 'pipe']`; the prompt is an argument, never stdin.
...
- **Dashboard questions use `swe-1.7-lightning`** rather than the account default, overridable with `SPRANG_DEVIN_MODEL`. Benchmarked end to end through the bridge on a question requiring an MCP call: ~23s against ~115s for `claude-sonnet-4.5`, and the fast model was also the strongest of its tier on a harder question (naming the files behind a specific change and summarising it), so the speedup costs nothing in quality. Dashboard questions are short lookups against the graph.
- **An authenticated `devin` CLI now outranks the in-editor `devin-local` hooks.** Hooks can only deliver when something happens in the session, so a question asked while the editor sits idle waits for the user to return; the CLI has no such gap. `devin-local` remains the default when no CLI is authenticated, and either can be chosen explicitly in the dashboard.
...
- **`devin-local` bridge — the highest-priority path, and the one that needs no extra login.** Delivered by two lifecycle hooks (`Stop`, `UserPromptSubmit`) that run *inside* your editor's Devin session, so the question is answered by the agent you are already talking to, with its context and MCP tools. The dashboard writes `.sprang/agent-question.md`; the first hook to fire consumes it (renaming to `.delivered.md`, so delivery is exactly-once and cannot cause a stop-loop). Verified end to end. Limitation: a hook runs only when a turn ends or you type, so a question asked while the session is idle waits for the next interaction.
- **(superseded during development) `devin-local` via an editor extension.** An extension calling `devin.sendChatActionMessage` was built and measured, then dropped. `explainAndFixProblem` opens a *new* conversation answered by Cascade with none of the session's context; `codeBlockMention` does reach the current conversation but only inserts text, and no command exists to submit the chat input. With both routes installed the extension also won the race against the hook every time, so dashboard questions were silently answered by the wrong agent. The hooks are strictly better on every axis: no install, full context, working MCP.
...
- **`.sprang/bridge.log`** — one line per dashboard-question event (`ask`, `devin.spawn`, `devin.exit`, `degrade`), with the chosen bridge, model, exit code and duration. The bridge spawns background processes whose failures were previously invisible: the panel simply span. Best-effort, and gitignored.
...
### Added (one-command install for every platform)

- **`sprang init --platform <claude|copilot|windsurf|all>`** now scaffolds a project in a single step: it writes the MCP config in the location each agent reads (`.mcp.json` for Claude Code, `.vscode/mcp.json` for Copilot, `.devin/config.json` with `${workspaceFolder}` for Windsurf/Devin) **with the resolved absolute server path**, and copies that platform's slash commands, rules, workflows, skills, and `merge.py` into the project. The universal install is now: `npm i -g @faviovazquez/sprang` → `sprang init --platform claude`. Without `--platform` it keeps the old behaviour (just `.mcp.json`) and prints a hint.
- **The npm package now actually ships the agent-integration files.** The `files` whitelist listed `.claude/`, `.windsurf/`, `.devin/`, etc., but those live at the repo root, not under `packages/cli/`, so npm silently omitted them — the published tarball contained only the CLI/MCP/dashboard, none of the slash commands, rules, or skills. `pack-assets.mjs` now copies them into `dist/agent-assets/` (dev-only `worktrees`/`node_modules` excluded) so the package is self-sufficient and `sprang init` can scaffold from it.

### Changed (`sprang merge` is now the primary, normalising assembler)

- **`sprang merge` (the TypeScript CLI) now applies the same schema normalisation as `merge.py`** via a shared `@sprang/core` `normalizeAssembledGraph()` function, reads `risk-scores.json` (risk score / factors / decision_context / warnings — which it previously ignored), and validates the result against `knowledgeGraphSchema` before writing. `/sprang-analyze` now **prefers `sprang merge`** (always available after the npm install, no Python needed) and falls back to `merge.py` only when the CLI isn't on `PATH` — so npm-installed users never depend on a Python file being copied into their project. Both assemblers produce identical, schema-valid graphs.

### Improved

- **`GraphLoader` now logs a concise, actionable reason when a graph fails to validate** instead of silently returning null (which surfaced only as `GRAPH_NOT_FOUND`). It writes a one-line summary — issue count plus the few most common `path :: message` shapes, with repeats collapsed (e.g. `domains.[].label: Required (×3)`) — and a hint to re-run `/sprang-analyze` or `sprang scan`. The `summarizeZodIssues` helper moved to `@sprang/core` so both the MCP loader and `sprang merge` share it.
...
- **+18 unit tests → 689** (was 671): `merge-normalize.test.ts` (6, `merge.py`) and `normalize-assembled.test.ts` (4, the shared TS normaliser) assert drifted agent output becomes schema-valid and data is preserved; `merge.test.ts` (+1) covers `sprang merge` normalisation + `risk-scores.json` application; `init.test.ts` (5) covers `--platform` scaffolding for all three agents (MCP config location/contents, command/rule/skill copy, `merge.py` present, no `worktrees` leakage, unknown-platform rejection); `graph-loader.test.ts` (+2) covers the invalid-graph null path and the `summarizeZodIssues` diagnostic. Lint/typecheck clean; full UI + bridge e2e green. Re-validated by re-running `/sprang-analyze` via live `claude -p` and confirming the produced graph loads through all 9 MCP tools.
...
### Added (npm distribution — 2026-06-13)

- **`sprang` is now a single self-contained npm package**, published as **`@faviovazquez/sprang`** (the unscoped `sprang` name is blocked by npm's package-name similarity guard; the scoped name installs the same `sprang` CLI command). Renamed from the internal `@sprang/cli` workspace name and published with `publishConfig.access: "public"`. tsup bundles `@sprang/core` inline (`noExternal`) so the installed package has no `workspace:*` deps. All platform integration files (`.claude/`, `.windsurf/`, `.devin/`, `.copilot-plugin/`, `.claude-plugin/`) are included via the `files` whitelist.
- **Standalone dashboard server** (`dist/dashboard-server.js`). The 9 Vite plugin route handlers (knowledge graph, diff overlay, agent bridge, analyze endpoints, health history) were extracted from `vite.config.ts` into a framework-agnostic `registerRoutes()` module (`packages/dashboard/src/server/routes.ts`). A new `standalone.ts` (`http.createServer` + MIME-aware static file serving + SPA fallback) is built by tsup alongside the dashboard Vite output. `sprang open` now spawns this standalone server in all install contexts (no Vite required). Finds the static directory via `SPRANG_DASHBOARD_DIST` env var (set automatically by `sprang open`), falling back to co-located `dashboard/` folder in the npm package.
- **MCP server CJS standalone bundle** (`dist/mcp-server.cjs`). The MCP package now produces a second, self-contained CJS build (tsup `splitting: false`, `noExternal: ['@sprang/core', '@modelcontextprotocol/sdk']`) that works when run via `node mcp-server.cjs` without any peer package installs. Used in `.mcp.json` for npm-installed users.
- **`sprang init` command.** New CLI subcommand that writes or merges `.mcp.json` into a target project: finds the MCP server path (npm layout: `dist/mcp-server.cjs`; monorepo layout: `packages/mcp/dist/server.cjs`) and sets `SPRANG_ROOT: "."`. Supports `--yes` flag for non-interactive use.
- **`scripts/pack-assets.mjs`** — build-time script (called by `publish.yml` CI and the `pack-and-install` smoke job) that copies `dashboard/dist/` → `dist/dashboard/`, `standalone.js` → `dist/dashboard-server.js`, and `mcp/dist/server.cjs` → `dist/mcp-server.cjs`. Automatically removes stale `.js` copies from previous builds to keep the tarball clean.
- **Automated npm publish workflow** (`.github/workflows/publish.yml`). Triggered on `v*` tags. Runs full CI as a prerequisite, verifies the git tag matches `package.json` version and all four package versions are in sync, copies assets, then publishes with provenance (`--provenance --access public`) using the `NPM_TOKEN` GitHub Actions secret. Token is never logged or written to files.
- **Pack-and-install CI smoke test** (`pack-and-install` job in `ci.yml`). Runs on every push (after `build-and-test`): packs the tarball with `npm pack`, installs into a clean `npm init -y` directory, and asserts: `sprang --version` returns the correct semver, `sprang scan --phase1-only` on a fixture TypeScript project produces a valid `knowledge-graph.json`, and the MCP server responds to a JSON-RPC `initialize` handshake.

### Fixed (2D graph pulse ring — 2026-06-17)

- **"Orphaned" pulse rings left floating in empty space.** Selecting a node draws an animated magenta pulse ring (a framer-motion overlay distinct from Sigma's built-in white node highlight). Two bugs left stale rings stranded off-node: (1) The exit animation inherited the ring's `transition={{ repeat: Infinity }}`, so when selection changed the old ring's exit animation **repeated forever and never completed** — `AnimatePresence` therefore never unmounted it, freezing it at its last screen position. Fixed by giving `exit` its own finite transition (`{ duration: 0.2, repeat: 0 }`) and moving the looping pulse transition inside the `animate` prop. (2) The ring position was computed only once (on selection change), so panning/zooming the camera left the ring frozen at stale viewport coordinates while the node moved away. Fixed by recomputing the ring's viewport position from the node's graph coordinates on every Sigma `afterRender` (cheap, only fires on camera changes/refresh), keeping it glued to the node. A `selectedNodeIdRef` lets the once-created renderer's event handler read the current selection.

### Fixed (publish hardening — 2026-06-17)

- **Removed orphaned `packages/cli/bin/install.js`.** A second, never-invoked implementation of `.mcp.json` writing shipped in the tarball (via `files: ["bin/"]`) but was not a `bin` entry and was referenced nowhere — and it used different flags (`--target`) than the real `sprang init` subcommand (`src/commands/init.ts`, positional `[path]` + `--yes`). Deleted the dead script and dropped `bin/` from the package `files` whitelist so `sprang init` (`init.ts`) is the single source of truth. Verified absent from the packed tarball.
- **Tightened the publish manifest-version check.** `publish.yml` previously version-matched `.claude-plugin/marketplace.json` and `.devin/config.json`, which by design carry no `version` field — producing a spurious `?` and a warning every publish. The check now runs only against the two genuinely versioned manifests (`.claude-plugin/plugin.json`, `.copilot-plugin/plugin.json`) and is a hard failure (`exit 1`) rather than a warning, so a real version drift can't slip through.
- **Verified the full published-package flow end-to-end.** Packed the `sprang` tarball (1.5 MB, 43 files), installed it into a clean `npm init -y` directory, and confirmed: `sprang --version` → `0.2.1`; `sprang init --yes` writes `.mcp.json` pointing at the bundled `dist/mcp-server.cjs`; `sprang scan --phase1-only` produces a valid `knowledge-graph.json`; the MCP server answers a JSON-RPC `initialize` handshake (`serverInfo.name: sprang`); and `sprang open` boots the standalone server, serving `/` (SPA, 200), `/knowledge-graph.json` (200), and `/bridge-status` (200). The `sprang` name is available on npm (registry 404).
...
- **Background Phase 2 never ran (`sprang scan` left graphs at `phase: skeleton`).** `runner.ts` forked the worker via `new URL('./phase2-runner.js', import.meta.url)`, but after bundling `runner.ts` is flattened into `core/dist/index.js` (dist root) while the worker stays at `dist/orchestrator/phase2-runner.js` — so the resolved path didn't exist. With `stdio: 'ignore'` the `ENOENT` was completely silent, meaning every `sprang scan` produced a skeleton-only graph (no layers, risk, smells, domains, tours, or security). Fixed by resolving whichever candidate path actually exists (source vs dist layout), adding a `child.on('error')` handler to surface spawn failures, and falling back to running Phase 2 inline if the worker can't be located. Verified end-to-end: background Phase 2 now completes (`phase: complete`, 9 layers, 17 security findings) with zero API keys.
- **Landing-screen "instant analysis" silently did nothing.** The dashboard `POST /analyze` endpoint (local path or GitHub URL) spawned `npx sprang scan`, but `sprang` is not published to npm — so `npx` hit the registry, got a 404, and failed silently (`stdio: 'ignore'`). The repo cloned but no graph was ever produced (the landing screen spun forever). Replaced with a CLI-resolution priority chain that works in every scenario: (1) local monorepo build `node <repo>/packages/cli/dist/index.js`, (2) `sprang` on PATH (global install / `pnpm link`), (3) `npx -y sprang` (published-to-npm fallback). Spawn errors now surface via a `child.on('error')` handler. Verified end-to-end: `POST /analyze` with a GitHub URL clones + scans and serves the graph within ~3s.
...
- **ESLint ignores agent git worktrees.** ESLint v9 flat config does not read `.gitignore`, so `pnpm lint` (`eslint .`) descended into the gitignored `.claude/worktrees/` agent worktree copies and re-reported the entire repo's lint state once per worktree (hundreds of spurious errors locally). CI was unaffected — a fresh checkout has no worktrees — but it broke `pnpm lint` for any developer using the agent worktree harness. Added `.claude/worktrees/**` to the config's `ignores` so local runs match CI regardless of worktree state.
