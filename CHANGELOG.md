...
- **`sprang` is now a single self-contained npm package.** CLI package renamed from `@sprang/cli` (scoped) to `sprang` (unscoped) and published with `publishConfig.access: "public"`. tsup bundles `@sprang/core` inline (`noExternal`) so the installed package has no `workspace:*` deps. All platform integration files (`.claude/`, `.windsurf/`, `.devin/`, `.copilot-plugin/`, `.claude-plugin/`) are included via the `files` whitelist.
