...
- **`devin-local` bridge — the highest-priority path, and the one that needs no extra login.** Devin local (Devin Desktop) is already authenticated, but it is not a spawnable process: its credentials live in the IDE, not in the CLI credential store, so `devin auth status` reports "Not logged in" while the IDE is signed in and working. The only way in is the editor command `devin.sendChatActionMessage`, which nothing outside the editor can invoke. `sprang-devin-bridge-0.3.0.vsix` (committed at the repo root) watches `.sprang/agent-question.md` and pushes it into the Devin chat; Devin answers with `sprang_respond` as usual. Detection requires the extension's `.sprang/.devin-bridge-active` marker — being inside Devin Desktop is not enough, since without the extension nothing is listening. Only the `.vsix` is committed; because a `.vsix` is a zip that includes `src/`, the source travels with the artifact and stays recoverable.
