...
- **`devin auth status` cannot be trusted to mean "unusable".** Devin Desktop authenticates the CLI over ACP (`method_id=windsurf-api-key`) rather than through the CLI's own credential store, so `auth status` reports "Not logged in" while the IDE works. The CLI accepts the same credential as `WINDSURF_API_KEY`, so exporting it (copy from the IDE via *Devin: Copy API Key*) activates the `devin` CLI bridge against the account you are already signed into — no second login. Detection now honours that env var.
