...
- **11 new unit tests** — 3 layer-violation cases (flags upward import, ignores downward, exempts util/test) and 8 call-graph/pattern cases (internal + external `calls` edges, caller counts, self-call exclusion, context_provider / event_emitter / decorator detection) via a new `call-graph` fixture. 1 new e2e test (landing screen detects GitHub URL input) plus updated landing-screen tests. Totals: 624 unit, 57 e2e.
