...
# Must mirror EDGE_TYPES in packages/core/src/schema/types.ts
VALID_EDGE_TYPES = [
...
VALID_EDGE_TYPE_SET = set(VALID_EDGE_TYPES)

# Agent-drift edge types → canonical. `True` means the alias points the opposite
# way from the canonical edge, so source/target must be swapped. The classic case
# is `tests` (test → source), whose canonical form is `tested_by` (source → test).
# Keep in sync with EDGE_TYPE_ALIASES in packages/core/src/graph/normalize-assembled.ts.
EDGE_TYPE_ALIASES = {
    # dependencies
    'tests': ('tested_by', True),
    'tested': ('tested_by', True),
    'test': ('tested_by', True),
    'testedby': ('tested_by', False),
    'dependson': ('depends_on', False),
    'depends': ('depends_on', False),
    'requires': ('depends_on', False),
    'uses': ('depends_on', False),
    'configuredby': ('configures', True),
    # structural
    'extends': ('inherits', False),
    'subclassof': ('inherits', False),
    'inheritsfrom': ('inherits', False),
    'importedby': ('imports', True),
    'imported': ('imports', True),
    'includes': ('contains', False),
    'containedby': ('contains', True),
    'partof': ('contains', True),
    'belongsto': ('contains', True),
    'parentof': ('contains', False),
    'childof': ('contains', True),
    'implementsinterface': ('implements', False),
    # behavioral
    'invokes': ('calls', False),
    'callsfunction': ('calls', False),
    'calledby': ('calls', True),
    'subscribesto': ('subscribes', False),
    'listensto': ('subscribes', False),
    'publishesto': ('publishes', False),
    'emits': ('publishes', False),
    # data flow
    'reads': ('reads_from', False),
    'readsfrom': ('reads_from', False),
    'queries': ('reads_from', False),
    'writes': ('writes_to', False),
    'writesto': ('writes_to', False),
    'persists': ('writes_to', False),
    # semantic
    'references': ('related', False),
    'refersto': ('related', False),
    'relatedto': ('related', False),
    'relatesto': ('related', False),
    'seealso': ('related', False),
    'similarto': ('similar_to', False),
    # knowledge
    'builds': ('builds_on', False),
    'buildson': ('builds_on', False),
    'derivedfrom': ('builds_on', False),
    'documentedby': ('documents', True),
    'describes': ('documents', False),
    'categorizedunder': ('categorized_under', False),
    'authoredby': ('authored_by', False),
    # infrastructure
    'deployedby': ('deploys', True),
    'servedby': ('serves', True),
    'triggeredby': ('triggers', True),
    'definesschema': ('defines_schema', False),
    'crossdomain': ('cross_domain', False),
    'containsflow': ('contains_flow', False),
    'flowstep': ('flow_step', False),
}
...
def _edge_key(raw):
    ...
    return ''.join(c for c in str(raw).strip().lower() if c.isalnum())


def normalize_edge(e):
    """Coerce one agent-written edge to the canonical schema, or None to drop it.

    knowledgeGraphSchema has no nullable fields, so an unmappable `type` makes the
    whole graph unloadable — dropping one edge beats losing the graph.
    """
    if not isinstance(e, dict):
        return None
    source = _str(e.get('source') if e.get('source') is not None else e.get('from'))
    target = _str(e.get('target') if e.get('target') is not None else e.get('to'))
    if not source or not target:
        return None

    raw_type = _str(e.get('type') or e.get('label') or e.get('kind'))
    key = _edge_key(raw_type)
    etype = None
    if raw_type in VALID_EDGE_TYPE_SET:
        etype = raw_type
    elif key in EDGE_TYPE_ALIASES:
        etype, swap = EDGE_TYPE_ALIASES[key]
        if swap:
            source, target = target, source
    else:
        # Last resort: canonical name with separators stripped (`depends_on` ≡ `dependson`).
    ...
    out = {'source': source, 'target': target, 'type': etype}
    if e.get('direction') in ('forward', 'backward', 'bidirectional'):
        out['direction'] = e['direction']
    if e.get('description') is not None:
        out['description'] = _str(e['description'])
    if isinstance(e.get('weight'), (int, float)) and not isinstance(e.get('weight'), bool):
        out['weight'] = e['weight']
...
def normalize_edges(raw_edges):
    """Normalise + dedupe an agent-written edge array. Returns (edges, dropped_counts)."""
    dropped = {}
    by_signature = {}
    for raw in raw_edges:
        ...
        if not edge:
            if isinstance(raw, dict):
                label = _str(raw.get('type') or raw.get('label') or raw.get('kind')) or '(missing type)'
            else:
                label = '(malformed)'
            dropped[label] = dropped.get(label, 0) + 1
            continue
        by_signature[f"{edge['source']}::{edge['target']}::{edge['type']}"] = edge
    return list(by_signature.values()), dropped


def apply_node_warnings(nodes, index):
    """Re-attach Phase 1 findings to agent-assembled nodes, in place.

    Existing values win — an agent-produced warning is deliberate enrichment and
    must not be clobbered by the older static scan. Returns how many nodes changed.
    Mirrors applyNodeWarnings in packages/core/src/graph/node-warnings.ts.
    """
    ...
    for node in nodes:
        if not isinstance(node, dict):
            continue
        entry = index.get(node.get('id'))
        if not isinstance(entry, dict):
            continue
        touched = False
        for key in ('structural_warnings', 'security_warnings', 'risk_score', 'risk_factors'):
            if key not in entry:
                continue
...
def risk_entries(raw):
    ...
    Mirrors riskEntries in packages/cli/src/commands/merge.ts.
    """
...
def main():
    ...
    # ── Edges (normalise types, then deduplicate) ─────────────────────────────
    ...
    edges, dropped_edge_types = normalize_edges(raw_edges)
    if dropped_edge_types:
        detail = ', '.join(f"{t} ({n})" for t, n in sorted(dropped_edge_types.items()))
        print(f"Warning: dropped edges with unmappable types: {detail}", file=sys.stderr)
    ...
    # Without this, enrichment silently drops every smell/security finding and the
    # health grade improves for code that did not change.
    warnings_path = os.path.join(inter, "node-warnings.json")
    if os.path.exists(warnings_path):
        try:
            warn_index = json.load(open(warnings_path))
            applied = apply_node_warnings(nodes, warn_index)
            if applied:
                print(f"Restored Phase 1 warnings on {applied} nodes")
        except Exception as e:
            print(f"Warning: could not load node-warnings.json: {e}", file=sys.stderr)
    ...
    if os.path.exists(risk_path):
        try:
            ...
            for node_id, risk_info in risk_entries(json.load(open(risk_path))):
                node = node_map_tmp.get(node_id)
                if node is None:
                    continue
    ...
    # `knowledge` graphs (markdown vaults) use the same envelope but a different
    # kind, which the dashboard reads to switch into knowledge view mode.
    graph_kind = os.environ.get("SPRANG_GRAPH_KIND") or meta.get("kind") or "codebase"
    if graph_kind not in ("codebase", "knowledge"):
        print(f"Warning: unknown kind {graph_kind!r}; using 'codebase'", file=sys.stderr)
        graph_kind = "codebase"
