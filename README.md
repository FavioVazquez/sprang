...
<p align="center">
  ...
  <a href="#mcp-tools"><img src="https://img.shields.io/badge/MCP-10_tools-7C3AED?style=flat-square" alt="10 MCP tools"/></a>
...
| Path | What it does |
|---|---|
| `.devin/mcp_config.json` | 10 MCP tools. `${workspaceFolder}` is resolved by Devin, so the config is portable. |
...
The first command registers the GitHub repo as a marketplace source (reads `.claude-plugin/marketplace.json`); the second installs the plugin. Then build the MCP server binary in the plugin cache to unlock the 10 tools:
...
| **10 MCP tools** | Direct graph access — all agents read and write the graph via MCP |
...
| Agent-readable (MCP) | ✅ 10 tools | — | via context | partial |
...
├── mcp/        stdio MCP server — 10 tools for all AI platforms
...
<p align="center">
  <img src="assets/mcp-tools.png" alt="Sprang MCP server — 10 tools for all AI platforms" width="100%" />
</p>
...
| `@sprang/mcp` | Vitest | GraphLoader (incl. `GRAPH_INVALID` diagnostics), all 10 tools |
