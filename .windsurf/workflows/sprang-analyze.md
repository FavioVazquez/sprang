...
**Step 3 — Run the merge script** (Python 3 stdlib only — works on any machine, no install needed):

```bash
PROJECT_ROOT="$PROJECT_ROOT" python3 "$(dirname "$0")/../.windsurf/skills/sprang-analyze/scripts/merge.py"
```

If the skills path isn't available (e.g. Claude Code or Copilot install without the `.windsurf/` tree), locate it via:
```bash
# Find merge.py — check all known install locations
MERGE_SCRIPT=""
for p in \
  "$(pwd)/.windsurf/skills/sprang-analyze/scripts/merge.py" \
  "$HOME/.windsurf/skills/sprang-analyze/scripts/merge.py" \
  "$HOME/.sprang/repo/.windsurf/skills/sprang-analyze/scripts/merge.py" \
  "$HOME/tools/sprang/.windsurf/skills/sprang-analyze/scripts/merge.py"; do
  [[ -f "$p" ]] && MERGE_SCRIPT="$p" && break
done

if [[ -z "$MERGE_SCRIPT" ]]; then
  echo "ERROR: merge.py not found. Run install.sh to set up Sprang." && exit 1
fi
...
**Do not write the graph file yourself. Do not run anything else after this.**
