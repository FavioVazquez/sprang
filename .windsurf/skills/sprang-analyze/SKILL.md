...
7. **Save** — run `sprang merge $PROJECT_ROOT --intermediate $PROJECT_ROOT/.sprang/intermediate` (if `sprang` not on PATH: `node ~/.sprang/repo/packages/cli/dist/index.js merge ...`). Do NOT write `knowledge-graph.json` manually.
